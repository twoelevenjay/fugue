...
// ============================================================================
// DEBUG CONVERSATION LOG — Full LLM conversation capture for Ramble
//
// Writes complete LLM request/response transcripts to:
//   .vscode/ramble/debug/<date>_<time>_<sessionId>.md
//
// This captures EVERYTHING Ramble sends to and receives from Copilot's
...
//   - Timestamp
//   - Phase (analysis, question-generation, merge, compilation, etc.)
//   - Model used
//   - Full prompt sent
...
export interface DebugLogEntry {
    ...
    timestamp: string;
    /** Which phase of prompt compilation */
    phase: DebugPhase;
    /** Label / description */
    label: string;
    /** Model identifier */
    ...
    error?: string;
    /** Question round number (if applicable) */
    questionRound?: number;
}

/**
 * Manages a debug conversation log file for a single Ramble session.
 *
 * Usage:
 *   const debugLog = new RambleDebugConversationLog();
 *   await debugLog.initialize();
 *   // ... during prompt compilation:
 *   await debugLog.logLLMCall({ ... });
 *   // at end:
 *   await debugLog.finalize('completed');
 */
export class RambleDebugConversationLog {
    ...
    constructor(enabled = true) {
        ...
        this.sessionId = `ramble-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    }
    ...
    async initialize(): Promise<boolean> {
        ...
        const debugDir = vscode.Uri.joinPath(rootUri, '.vscode', 'ramble', 'debug');
        ...
        // Write the header
        this.buffer.push(`# Ramble Debug Log — ${this.sessionId}\n`);
     ...
     * Call this after the LLM response has been fully received.
     */
    async logLLMCall(entry: DebugLogEntry): Promise<void> {
        ...
        if (entry.questionRound !== undefined) {
            section.push(`| **Question Round** | ${entry.questionRound} |`);
        }
        if (entry.error) {
     ...
     */
    async logEvent(phase: DebugPhase | string, message: string): Promise<void> {
        if (!this.enabled || !this.initialized) {
            return;
        }
     ...
     */
    async finalize(
        outcome: 'completed' | 'failed' | 'cancelled' | 'reset',
        errorMessage?: string,
    ): Promise<void> {
        if (!this.enabled || !this.initialized) {
            return;
        ...
        const byPhase = new Map<DebugPhase | string, { count: number; totalMs: number }>();
