...
export class ProgressDashboard {
  ...
  constructor(opts: {
    ...
    this.startTime =
      typeof opts.startTime === 'number'
        ? opts.startTime
        : (opts.startTime ? (opts.startTime as Date).getTime() : Date.now());
  }
  ...
  private renderStreams(state: AutonomousRunStateLike): string {
    ...
    const rows = streams.map((s) => {
      ...
      const progress = this.renderBar((s?.progressPercent as number) ?? this.deriveStreamPercent(s));
    ...
    const header = '| Stream | Branch | Status | Progress | Subtask | Last Activity |\n|---|---|---|---|---|---|';
    return `## Work Streams\n\n${header}\n${rows.join('\n')}`;
  }

  private deriveStreamPercent(s: any): number {
    const total = (s?.totalSubtasks as number) ?? (s?.subtasks?.length as number) ?? 0;
    const done = (s?.completedSubtasks as number) ?? (s?.subtasks?.filter((x: any) => x?.status === 'completed')?.length as number) ?? 0;
  ...
  private renderNextUp(state: AutonomousRunStateLike): string {
    ...
    const lines = queue.slice(0, 10).map((q) => `- ${q?.title ?? q?.id ?? 'subtask'} (${q?.streamId ?? 'global'})`);
    return `## Next Up\n\n${lines.join('\n')}`;
  }
  ...
  private async writeProgress(content: string): Promise<void> {
    try {
      ...
      try {
        ...
        try { await vscode.workspace.fs.delete(final, { recursive: false, useTrash: false }); } catch {}
        await vscode.workspace.fs.rename(tmp, final, { overwrite: true });
      }
      this.logger.info('ProgressDashboard: updated PROGRESS.md');
    } catch (err: any) {
      this.logger.error(`ProgressDashboard: failed to write dashboard: ${String(err?.message ?? err)}`);
    }
  }
}
