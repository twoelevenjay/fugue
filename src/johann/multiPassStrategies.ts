...
export interface MultiPassStep {
    /** Role of this pass */
    role: 'draft' | 'critique' | 'revise' | 'sample' | 'aggregate' | 'verify' | 'repair' | 'extract' | 'score';
...
export interface MultiPassStrategy {
    ...
    /** Aggregation method */
    aggregator: {
        type: 'majority-vote' | 'consensus-threshold' | 'tool-oracle' | 'rubric-score' | 'critic-fixes';
        description: string;
    ...
    passes: [
        {
            ...
            instruction: 'Create an initial draft. Focus on covering all requirements and being thorough. Do not worry about polish yet.',
        },
        {
            role: 'critique',
...
Output a structured list of issues with severity (must-fix / nice-to-have) and specific location references.`,
        },
        {
            ...
            outputFormat: 'text',
            instruction: 'Revise the draft to address ALL must-fix issues. Show a clear mapping of issue → fix. Maintain the original intent while improving clarity and completeness.',
        },
    ],
    aggregator: {
        type: 'critic-fixes',
        description: 'Verify that revision addresses all must-fix issues from critique. If not, fail and escalate.',
        config: {
    ...
    description: 'Best for subjective quality improvements. Draft → Critique with rubric → Revise with explicit fixes.',
};
...
export const SELF_CONSISTENCY: MultiPassStrategy = {
    ...
    passes: [
        {
            ...
            instruction: 'Analyze the problem and provide your answer with reasoning and evidence. Be specific and cite line numbers, file names, or error messages where relevant.',
        },
        {
            role: 'aggregate',
            targetCost: 0,
    ...
    aggregator: {
        type: 'majority-vote',
        description: 'Sample N=3-5 times. Cluster answers by similarity. Pick the cluster with highest frequency AND highest average confidence. If no majority (< 50%), escalate.',
        config: {
            numSamples: 3,
    ...
    description: 'Sample multiple reasoning paths, then vote on consensus. Best for debugging and analysis with uncertain answers.',
};
...
export const TOOL_VERIFIED_LOOP: MultiPassStrategy = {
    ...
    passes: [
        {
            ...
            outputFormat: 'code',
            instruction: 'Generate the code or fix. Ensure it compiles and follows project conventions.',
        },
        {
            role: 'verify',
            targetCost: 0,
            instruction: 'Run automated checks: compile, typecheck, lint, tests. This is a tool-based verification step.',
        },
        {
            role: 'repair',
            targetCost: 0, // GPT-4.1 for diagnosis + GPT-5 mini for fix
            outputFormat: 'code',
            instruction: 'Given the automated check failures, diagnose the issue and generate a targeted fix. Focus ONLY on the reported errors. Do not refactor unrelated code.',
        },
    ],
    aggregator: {
        type: 'tool-oracle',
        description: 'Pass/fail is determined by automated tools. Loop up to maxIterations=3. If still failing after max iterations, escalate.',
        config: {
            maxIterations: 3,
    ...
    description: 'Generate code → Run tools (tests, lint, typecheck) → Repair if failed. Loop until pass or max iterations. Highest "truth signal".',
};
...
Deduplicate issues with same file + line + ruleId. Assign confidence based on certainty.`,
    ...
    aggregator: {
        type: 'rubric-score',
        description: 'Deduplicate by file+line+ruleId. Filter by minimum confidence. Group by severity. If any critical issues with high confidence, escalate for review.',
        config: {
    ...
    description: 'Extract issues → Score with rubric → Deduplicate. More stable than single-pass review.',
};
...
export function voteOnConsistency(
    ...
    const clusterScores = Array.from(clusters.entries()).map(([answer, clusterSamples]) => {
        ...
        const avgConfidence = clusterSamples.reduce((sum, s) => sum + s.confidence, 0) / clusterSamples.length;
...
export function verifyRevisionAddressesIssues(
    ...
    for (const issue of mustFixIssues) {
        ...
        const issueMentioned = revision.toLowerCase().includes(issue.issue.toLowerCase().slice(0, 20));
